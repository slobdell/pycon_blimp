...
_RB_INLINE void set_playmode_pulses2(unsigned long* width) {
	int i;
    ...
    for (i=0; i<32; i++)
    {
        switch (mixwidth[i])
        {
            ...
            default:
                ...
                if ((width[i] < RCSERVO_svParams[i].minDuty) || (width[i] > RCSERVO_svParams[i].maxDuty)) break;  // not deal with wrong width settings

                tmp = (long)width[i] + mixwidth[i];
                if (tmp < (long)RCSERVO_svParams[i].minDuty)  // avoid underflow
                    width[i] = RCSERVO_svParams[i].minDuty;
                else
                if (tmp > (long)RCSERVO_svParams[i].maxDuty)  // avoid overflow
                    width[i] = RCSERVO_svParams[i].maxDuty;
                else
                    width[i] = (unsigned long)tmp;
                break;
        }
    } //end for i...
}
...
RBAPI(int) rcservo_PlayAction(void) {
    ...
    {
        stop_playmode_pulses();                             // time-consuming function
        t = timer_nowtime() - RCSERVO_curaction_starttime;  // recalculate to get more precise timing
        ...
        set_playmode_pulses2(RCSERVO_midaction);
    ...
    if (RCSERVO_playState == RCSERVO_PLAYING)
    {
        ...
        if (t >= (RCSERVO_curaction_nexttime - RCSERVO_curaction_starttime))
        {
            ...
            if (t >= tt)
            {
                for (i=0; i<32; i++) RCSERVO_midaction[i] = RCSERVO_curaction[i];
                RCSERVO_playState = RCSERVO_PLAYEND;
                lasttime = nowtime; lasttime_used = true;
            }
            else
            {
                for (i=0; i<32; i++)
...
RBAPI(bool) rcservo_SetServo(int channel, unsigned servono) {
	...
	switch (servono)
	{
		...
		case RCSERVO_KONDO_KRS4014:
			...
			RCSERVO_svParams[channel].fbReadMode = RCSERVO_FB_FASTMODE;
            //rcservo_SetServoParams1(channel, 8000L, 700L, 2300L);
            rcservo_SetServoParams1(channel, 8000L, 450L, 2500L);
            rcservo_SetServoParams2(channel, 3500L, 200L);          // note that using the safe-mode feedback method, KRS-4014 will return a feedback pulse again 
                                                                    // after the dummy invalid pulse; so the safe-mode feedback method is not suggested to be used

            rcservo_SetReadFBParams1(channel, 50L, 250L, 2800L);
            rcservo_SetReadFBParams2(channel, 10L, ROBOARD_WOFFSET-10L);
			break;

/*
		case RCSERVO_KONDO_KRS4034:

            rcservo_SetServo(channel, RCSERVO_KONDO_COMMON);

			RCSERVO_svParams[channel].fbReadMode = RCSERVO_FB_FASTMODE;
            rcservo_SetServoParams1(channel, 8000L, 700L, 2300L);
            rcservo_SetServoParams2(channel, 3000L, 200L);          // note that KRS-4034 has no invalid duty and so the safe-mode feedback method 
                                                                    // cannot be used on KRS-4034
...
*/
		...
		case RCSERVO_GWS_S777:

            rcservo_SetServo(channel, RCSERVO_SERVO_DEFAULT_NOFB);
            rcservo_SetServoParams1(channel, 3500L, 600L, 2350L);
            break;

		case RCSERVO_GWS_S03T:
		case RCSERVO_GWS_MICRO:

            rcservo_SetServo(channel, RCSERVO_SERVO_DEFAULT_NOFB);
            rcservo_SetServoParams1(channel, 3500L, 580L, 2540L);
            break;
		...
		case RCSERVO_DMP_RS1270:

            rcservo_SetServo(channel, RCSERVO_SERVO_DEFAULT_NOFB);
            rcservo_SetServoParams1(channel, 4000L, 670L, 2230L);
			break;
