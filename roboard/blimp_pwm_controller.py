...
class BlimpPWMController(BasePWMController):
    ...
    def tick(self):
        # update the motor values based on instructions
        # send it to motors
        self._tick_pitch()
        self._tick_thrust()
        self._tick_yaw()
    ...
    def update_from_instructions(self, instructions):
        for instruction, intensity in instructions:
            if instruction == Instructions.PITCH_UP:
                # control elevator before moving to thrust vector
                elevator_intensity = intensity if intensity <= 0.5 else 0.5
                elevator_intensity *= 2
                self.current_elevator_angle = elevator_intensity * BlimpCalibrations.MAX_ELEVATOR_ANGLE

                if intensity <= 0.5:
                    self.current_thrust_vector_angle = BlimpCalibrations.MIN_THRUST_VECTOR_ANGLE
                else:
                    thrust_vector_intensity = intensity - 0.5
                    thrust_vector_intensity *= 2
                    self.current_thrust_vector_angle = BlimpCalibrations.MAX_THRUST_VECTOR_ANGLE * thrust_vector_intensity

            elif instruction == Instructions.PITCH_DOWN:
                # control elevator before moving to thrust vector
                elevator_intensity = intensity if intensity <= 0.5 else 0.5
                elevator_intensity *= 2
                self.current_elevator_angle = elevator_intensity * BlimpCalibrations.MIN_ELEVATOR_ANGLE
                self.current_thrust_vector_angle = BlimpCalibrations.MIN_THRUST_VECTOR_ANGLE

            elif instruction == Instructions.HOVER:
                self.current_elevator_angle = 0
                self.current_thrust_vector_angle = BlimpCalibrations.MAX_THRUST_VECTOR_ANGLE

            elif instruction == Instructions.SPEED:
                self.current_thrust_percent = intensity

            elif instruction == Instructions.YAW_LEFT:
                self.current_rudder_angle = intensity * BlimpCalibrations.MAX_RUDDER_ANGLE
            elif instruction == Instructions.YAW_RIGHT:
