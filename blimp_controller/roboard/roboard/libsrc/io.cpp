...
static unsigned long IO_dramClk = 300L;
RBAPI(unsigned long) io_GetDRAMCLK(void) {
    return IO_dramClk;
}

static unsigned long IO_cpuClk = 1000L;
RBAPI(unsigned long) io_GetCPUCLK(void) {
    return IO_cpuClk;
}
...
RBAPI(int) io_Init(void) {
    static unsigned long IO_dramCLKTable[] = { 166L, 200L, 233L, 266L, 300L, 333L, 366L, 400L, 
                                               416L, 433L, 450L, 466L, 483L, 500L, 516L, 533L };
    static unsigned long IO_cpuCLKTable1[] = { 0L,    0L,    500L,  600L,  700L,  800L,  833L,  866L,   // for Vortex86DX ver. D
                                               900L,  933L,  966L,  1000L, 1033L, 1066L, 1100L, 1133L, 
                                               1166L, 1200L, 1233L, 1266L, 1300L, 1333L, 1366L, 1400L, 
                                               1433L, 1466L, 1500L };
    static unsigned long IO_cpuCLKTable2[] = { 0L,    0L,    500L,  600L,  700L,  800L,  900L,  933L,   // for Vortex86DX ver. A/B/C
                                               966L,  1000L, 1033L, 1066L, 1066L, 1066L, 1100L, 1133L, 
                                               1166L, 1200L, 1233L, 1266L, 1300L, 1333L, 1366L, 1400L, 
                                               1433L, 1466L, 1500L };
    static unsigned long IO_cpuCLKTable3[] = { 500L,  600L,  700L,  833L,  866L,  900L,  933L,  966L,   // for Vortex86MX/MX+/DX2
                                               1000L, 1033L, 1066L, 1100L, 1133L, 1166L, 1200L, 800L };
    unsigned long strapreg;
    int i;
    ...
    strapreg = read_nb_reg(0x60);
    if ((io_CpuID() == CPU_VORTEX86DX_1) || (io_CpuID() == CPU_VORTEX86DX_2) || (io_CpuID() == CPU_VORTEX86DX_3))
    {
        IO_dramClk = IO_dramCLKTable[(int)((strapreg >> 8) & 0x07L)];
        
        if ((strapreg & 0x1fL) == 1L)
            IO_cpuClk  = IO_dramClk;
        else
        if (io_CpuID() == CPU_VORTEX86DX_3)
            IO_cpuClk  = IO_cpuCLKTable1[(int)(strapreg & 0x1fL)];
        else
            IO_cpuClk  = IO_cpuCLKTable2[(int)(strapreg & 0x1fL)];
    }
    else
    if ((io_CpuID() == CPU_VORTEX86MX) || (io_CpuID() == CPU_VORTEX86MX_PLUS))
    {
        IO_dramClk = IO_dramCLKTable[(int)((strapreg >> 8) & 0x0fL)];
        IO_cpuClk  = IO_cpuCLKTable3[(int)(strapreg & 0x0fL)];
    }
